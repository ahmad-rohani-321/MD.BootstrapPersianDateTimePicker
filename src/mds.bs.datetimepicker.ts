...
type CalendarSystem = 'gregorian' | 'jalali' | 'islamic';
...
export class MdsPersianDateTimePicker {
  ...
  private static toIslamic(gy: number, gm: number, gd: number): IslamicJsonModel {
    return this.d2i(this.g2d(gy, gm, gd));
  }
  private static islamicToGregorian(iy: number, im: number, id: number): GregorianJsonModel {
    return this.d2g(this.i2d(iy, im, id));
  }
  ...
  private static i2d(iy: number, im: number, id: number): number {
    return id + Math.ceil(29.5 * (im - 1)) + (iy - 1) * 354 + this.div(3 + 11 * iy, 30) + 1948439;
  }
  private static d2i(jdn: number): IslamicJsonModel {
    const iy = this.div(30 * (jdn - 1948439) + 10646, 10631);
    const im = Math.min(12, Math.ceil((jdn - 29 - this.i2d(iy, 1, 1)) / 29.5) + 1);
    const id = jdn - this.i2d(iy, im, 1) + 1;
    return {
      iy: iy,
      im: im,
      id: id
...
{{daysHtml}}
  ...
  private static previousYearTextPersian = 'مخکنی کال';
  private static previousMonthTextPersian = 'مخکنی میاشت';
  private static previousTextPersian = 'مخکي';
  private static nextYearTextPersian = 'وروسته کال';
  private static nextMonthTextPersian = 'وروسنتی میاشت';
  private static nextTextPersian = 'وروسته';
  private static todayTextPersian = 'نن ورځ';
  private static goTodayTextPersian = 'نن ورځ';
  private static cancelTextPersian = 'لغوه';
  private static currentYearTextPersian = 'جاري کال';
  ...
  private static shortDayNamesIslamic = [
    'ی',
    'د',
    'س',
    'څ',
  ...
  private static monthNamesIslamic = [
    'محرم',
    'صفر',
    'ربيع الأول',
    'ربيع الآخر',
  ...
  private static weekDayNamesIslamic = [
    'يکشنبه',
    'دوشنبه',
    'سې شنبه',
    'څلرنۍ',
  ...
  private static getDateTimeJsonIslamic1(dateTime: Date): GetDateTimeJson1 {
    const islamicDate = this.toIslamic(dateTime.getFullYear(), dateTime.getMonth() + 1, dateTime.getDate());
    return {
      year: islamicDate.iy,
      month: islamicDate.im,
      day: islamicDate.id,
      hour: dateTime.getHours(),
      minute: dateTime.getMinutes(),
      second: dateTime.getSeconds(),
  ...
  private static getDateTimeJsonIslamic2(yearIslamic: number, monthIslamic: number, dayIslamic: number, hour: number, minute: number, second: number): GetDateTimeJson1 {
    if (!this.isNumber(hour)) hour = 0;
    if (!this.isNumber(minute)) minute = 0;
    if (!this.isNumber(second)) second = 0;
    const gregorian = this.islamicToGregorian(yearIslamic, monthIslamic, dayIslamic);
    return MdsPersianDateTimePicker.getDateTimeJsonIslamic1(new Date(gregorian.gy, gregorian.gm - 1, gregorian.gd, hour, minute, second));
  }
  private static getCalendarSystem(setting: MdsPersianDateTimePickerSetting): CalendarSystem {
    if (setting.calendarSystem)
      return setting.calendarSystem;
    return setting.isGregorian ? 'gregorian' : 'jalali';
  }
  private static isGregorianCalendar(system: CalendarSystem): boolean {
    return system === 'gregorian';
  }
  private static isRtlCalendar(system: CalendarSystem): boolean {
    return system !== 'gregorian';
  }
  private static getDateTimeJsonByCalendar(dateTime: Date, system: CalendarSystem): GetDateTimeJson1 {
    if (system === 'gregorian') return this.getDateTimeJson1(dateTime);
    if (system === 'islamic') return this.getDateTimeJsonIslamic1(dateTime);
    return this.getDateTimeJsonPersian1(dateTime);
  }
  private static getDateTimeJsonByCalendar2(year: number, month: number, day: number, hour: number, minute: number, second: number, system: CalendarSystem): GetDateTimeJson1 {
    if (system === 'gregorian')
      return {
        year: year,
        month: month,
        ...
        second: second,
        millisecond: 0,
        dayOfWeek: new Date(year, month - 1, day, hour, minute, second).getDay()
      };
    if (system === 'islamic') return this.getDateTimeJsonIslamic2(year, month, day, hour, minute, second);
    return this.getDateTimeJsonPersian2(year, month, day, hour, minute, second);
  }
  private static getWeekDayName(englishWeekDayIndex: number, system: CalendarSystem): string {
    if (system === 'islamic') return this.weekDayNamesIslamic[englishWeekDayIndex];
    if (system === 'jalali') return this.weekDayNamesPersian[englishWeekDayIndex];
    return this.weekDayNames[englishWeekDayIndex];
  }
  private static getMonthName(monthIndex: number, system: CalendarSystem): string {
    if (monthIndex < 0)
      monthIndex = 11;
    else if (monthIndex > 11)
      monthIndex = 0;
    if (system === 'islamic') return this.monthNamesIslamic[monthIndex];
    if (system === 'jalali') return this.monthNamesPersian[monthIndex];
    return this.monthNames[monthIndex];
  }
  private static getWeekDayShortName(englishWeekDayIndex: number, system: CalendarSystem): string {
    if (system === 'islamic')
      return this.shortDayNamesIslamic[englishWeekDayIndex];
    if (system === 'jalali')
  ...
  private static getDaysInMonthIslamic(year: number, month: number): number {
    if (month === 12)
      return this.isLeapIslamicYear(year) ? 30 : 29;
    return month % 2 === 1 ? 30 : 29;
  }
  private static isLeapIslamicYear(year: number): boolean {
    return ((11 * year) + 14) % 30 < 11;
  }
  private static getDaysInMonthByCalendar(year: number, month: number, system: CalendarSystem): number {
    if (system === 'gregorian') return this.getDaysInMonth(year, month - 1);
    if (system === 'islamic') return this.getDaysInMonthIslamic(year, month);
    return this.getDaysInMonthPersian(year, month);
  }
  private static getLastDayDateOfPreviousMonth(dateTime: Date, system: CalendarSystem): Date {
    ...
    let dateTimeJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(dateTimeLocal, system);
    dateTimeJson.month += -1;
    if (dateTimeJson.month <= 0) {
      dateTimeJson.month = 12;
      dateTimeJson.year--;
    } else if (dateTimeJson.month > 12) {
      dateTimeJson.year++;
      dateTimeJson.month = 1;
    }
    return MdsPersianDateTimePicker.getDateTimeByCalendar(dateTimeJson.year, dateTimeJson.month, MdsPersianDateTimePicker.getDaysInMonthByCalendar(dateTimeJson.year, dateTimeJson.month, system), system);
  }
  private static getFirstDayDateOfNextMonth(dateTime: Date, system: CalendarSystem): Date {
    let dateTimeLocal = MdsPersianDateTimePicker.getClonedDate(dateTime);
    if (system === 'gregorian') {
    ...
    if (dateTimeJson.month > 12) {
      dateTimeJson.year++;
      dateTimeJson.month = 1;
    }
    return MdsPersianDateTimePicker.getDateTimeByCalendar(dateTimeJson.year, dateTimeJson.month, 1, system);
  }
  ...
  private static getDateTimeIslamic(yearIslamic: number, monthIslamic: number, dayIslamic: number, hour?: number, minute?: number, second?: number): Date {
    if (!this.isNumber(hour)) hour = 0;
    if (!this.isNumber(minute)) minute = 0;
    if (!this.isNumber(second)) second = 0;
    const gregorian = this.islamicToGregorian(yearIslamic, monthIslamic, dayIslamic);
  ...
  private static getDateTimeIslamic2(dateTimeJsonIslamic: GetDateTimeJson1): Date {
    if (!dateTimeJsonIslamic.hour) dateTimeJsonIslamic.hour = 0;
    if (!dateTimeJsonIslamic.minute) dateTimeJsonIslamic.minute = 0;
    if (!dateTimeJsonIslamic.second) dateTimeJsonIslamic.second = 0;
    const gregorian = this.islamicToGregorian(dateTimeJsonIslamic.year, dateTimeJsonIslamic.month, dateTimeJsonIslamic.day);
    return new Date(gregorian.gy, gregorian.gm - 1, gregorian.gd, dateTimeJsonIslamic.hour, dateTimeJsonIslamic.minute, dateTimeJsonIslamic.second);
  }
  private static getDateTimeByCalendar(year: number, month: number, day: number, system: CalendarSystem, hour?: number, minute?: number, second?: number): Date {
    if (system === 'gregorian') return new Date(year, month - 1, day, hour ?? 0, minute ?? 0, second ?? 0);
    if (system === 'islamic') return this.getDateTimeIslamic(year, month, day, hour, minute, second);
    return this.getDateTime1(year, month, day, hour, minute, second);
  }
  private static getDateTimeFromCalendarJson(dateTimeJson: GetDateTimeJson1, system: CalendarSystem): Date {
    if (system === 'gregorian') return this.getDateTime3(dateTimeJson);
    if (system === 'islamic') return this.getDateTimeIslamic2(dateTimeJson);
    return this.getDateTime2(dateTimeJson);
  }
  private static getDateTime4(dateNumber: number, dateTime: Date, system: CalendarSystem): Date {
    let dateTimeJson = MdsPersianDateTimePicker.getDateTimeJson2(dateNumber);
    if (system !== 'gregorian') {
      let dateTimeJsonCurrent = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(dateTime, system);
      dateTimeJsonCurrent.year = dateTimeJson.year;
      dateTimeJsonCurrent.month = dateTimeJson.month;
      dateTimeJsonCurrent.day = dateTimeJson.day;
      dateTime = this.getDateTimeFromCalendarJson(dateTimeJsonCurrent, system);
    } else
  ...
  private static getLesserDisableBeforeDate(setting: MdsPersianDateTimePickerSetting): GetDateTimeJson1 | null {
    ...
    if (resultDate == null)
      return null;
    return MdsPersianDateTimePicker.getDateTimeJsonByCalendar(resultDate, this.getCalendarSystem(setting));
  }
  ...
  private static addMonthToDateTimeJson(dateTimeJson: GetDateTimeJson1, addedMonth: number, system: CalendarSystem): GetDateTimeJson1 {
  ...
  private static getAmPm(hour: number, system: CalendarSystem): string {
    let amPm;
    if (hour > 12) {
      if (system === 'gregorian')
        amPm = 'PM';
  ...
  private static addMonthToDateTime(dateTime: Date, addedMonth: number, system: CalendarSystem): Date {
    let dateTimeJson: GetDateTimeJson1;
    if (system !== 'gregorian') {
      dateTimeJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(dateTime, system);
      dateTimeJson = MdsPersianDateTimePicker.addMonthToDateTimeJson(dateTimeJson, addedMonth, system);
      return this.getDateTimeFromCalendarJson(dateTimeJson, system);
    }
  ...
  private static getDateTimeString(dateTimeJson: GetDateTimeJson1, format: string, system: CalendarSystem, persianNumber: boolean): string {
    ...
    format = format.replace(/MMMM/mg, MdsPersianDateTimePicker.getMonthName(dateTimeJson.month - 1, system));
    ...
    format = format.replace(/dddd/mg, MdsPersianDateTimePicker.getWeekDayName(dateTimeJson.dayOfWeek, system));
    ...
    format = format.replace(/tt/mg, this.getAmPm(dateTimeJson.hour, system));
    format = format.replace(/t/mg, this.getAmPm(dateTimeJson.hour, system)[0]);
  ...
  private static getSelectedDateTimeTextFormatted(setting: MdsPersianDateTimePickerSetting): string {
    const calendarSystem = this.getCalendarSystem(setting);
    ...
    if (setting.rangeSelector && setting.rangeSelectorStartDate != undefined && setting.rangeSelectorEndDate != undefined)
      return MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(setting.rangeSelectorStartDate, calendarSystem), setting.textFormat, calendarSystem, setting.persianNumber) + ' - ' +
        MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(setting.rangeSelectorEndDate, calendarSystem), setting.textFormat, calendarSystem, setting.persianNumber);
    return MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(setting.selectedDate, calendarSystem), setting.textFormat, calendarSystem, setting.persianNumber);
  }
  private static getSelectedDateFormatted(setting: MdsPersianDateTimePickerSetting): string {
    ...
    if ((!setting.rangeSelector && !setting.selectedDate) ||
      ...
      return '';
    if (setting.rangeSelector)
      return MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJson1(setting.rangeSelectorStartDate!), setting.dateFormat, 'gregorian', setting.persianNumber) + ' - ' +
        MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJson1(setting.rangeSelectorEndDate!), setting.dateFormat, 'gregorian', setting.persianNumber);
    return MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJson1(setting.selectedDate!), setting.dateFormat, 'gregorian', setting.persianNumber);
  }
  private static getDisabledDateObject(setting: MdsPersianDateTimePickerSetting): [GetDateTimeJson1 | null, GetDateTimeJson1 | null] {
    ...
    if ((setting.fromDate || setting.toDate) && setting.groupId) {
      ...
      disableAfterDateTimeJson = !toDateSelectedDate ? null : MdsPersianDateTimePicker.getDateTimeJsonByCalendar(toDateSelectedDate, this.getCalendarSystem(setting));
      ...
      disableBeforeDateTimeJson = !fromDateSelectedDate ? null : MdsPersianDateTimePicker.getDateTimeJsonByCalendar(fromDateSelectedDate, this.getCalendarSystem(setting));
    }
    }
  ...
  private getYearsBoxBodyHtml(setting: MdsPersianDateTimePickerSetting, yearToStart: number): MdsPersianDateTimePickerYearToSelect {
    ...
    const calendarSystem = MdsPersianDateTimePicker.getCalendarSystem(setting);
    ...
    selectedDateTimeToShowJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(selectedDateToShow, calendarSystem);
    todayDateTimeJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(new Date(), calendarSystem);
    counter = 1;
    ...
    for (let i = yearStart; i < yearEnd; i++) {
      ...
      let currentYearDateTimeJson = MdsPersianDateTimePicker.getDateTimeJson2(MdsPersianDateTimePicker.convertToNumber2(i, selectedDateTimeToShowJson.month, MdsPersianDateTimePicker.getDaysInMonthByCalendar(i, selectedDateTimeToShowJson.month, calendarSystem)));
      let currentYearDisabledAttr = '';
      let yearText = MdsPersianDateTimePicker.isRtlCalendar(calendarSystem) ? MdsPersianDateTimePicker.toPersianNumber(i) : i.toString();
      ...
      if (todayAttr)
        selectedYearTitle = MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.currentYearText : MdsPersianDateTimePicker.currentYearTextPersian;
...
`;
    ...
    html = html.replace(/\{\{cancelText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.cancelText : MdsPersianDateTimePicker.cancelTextPersian);
  ...
  private getYearsBoxHeaderHtml(setting: MdsPersianDateTimePickerSetting, yearStart: number, yearEnd: number): string {
    ...
    html = html.replace(/\{{rtlCssClass\}\}/img, MdsPersianDateTimePicker.isRtlCalendar(calendarSystem) ? 'rtl' : '');
    html = html.replace(/\{{dirAttrValue\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? 'ltr' : 'rtl');
    html = html.replace(/\{\{yearsRangeText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? yearsRangeText : MdsPersianDateTimePicker.toPersianNumber(yearsRangeText));
    html = html.replace(/\{\{previousText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.previousText : MdsPersianDateTimePicker.previousTextPersian);
    html = html.replace(/\{\{nextText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.nextText : MdsPersianDateTimePicker.nextTextPersian);
  ...
  private getDateTimePickerMonthHtml(setting: MdsPersianDateTimePickerSetting, isNextMonth: boolean, isPrevMonth: boolean): string {
    ...
    html = html.replace(/\{\{weekDayShortName1CssClass\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? 'text-danger' : '');
    html = html.replace(/\{\{weekDayShortName7CssClass\}\}/img, !MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? 'text-danger' : '');
    html = html.replace(/\{\{previousYearText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.previousYearText : MdsPersianDateTimePicker.previousYearTextPersian);
    html = html.replace(/\{\{previousMonthText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.previousMonthText : MdsPersianDateTimePicker.previousMonthTextPersian);
    html = html.replace(/\{\{nextMonthText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.nextMonthText : MdsPersianDateTimePicker.nextMonthTextPersian);
    html = html.replace(/\{\{nextYearText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.nextYearText : MdsPersianDateTimePicker.nextYearTextPersian);
    html = html.replace(/\{\{monthName1\}\}/img, MdsPersianDateTimePicker.getMonthName(0, calendarSystem));
    html = html.replace(/\{\{monthName2\}\}/img, MdsPersianDateTimePicker.getMonthName(1, calendarSystem));
    html = html.replace(/\{\{monthName3\}\}/img, MdsPersianDateTimePicker.getMonthName(2, calendarSystem));
    html = html.replace(/\{\{monthName4\}\}/img, MdsPersianDateTimePicker.getMonthName(3, calendarSystem));
    html = html.replace(/\{\{monthName5\}\}/img, MdsPersianDateTimePicker.getMonthName(4, calendarSystem));
    html = html.replace(/\{\{monthName6\}\}/img, MdsPersianDateTimePicker.getMonthName(5, calendarSystem));
    html = html.replace(/\{\{monthName7\}\}/img, MdsPersianDateTimePicker.getMonthName(6, calendarSystem));
    html = html.replace(/\{\{monthName8\}\}/img, MdsPersianDateTimePicker.getMonthName(7, calendarSystem));
    html = html.replace(/\{\{monthName9\}\}/img, MdsPersianDateTimePicker.getMonthName(8, calendarSystem));
    html = html.replace(/\{\{monthName10\}\}/img, MdsPersianDateTimePicker.getMonthName(9, calendarSystem));
    html = html.replace(/\{\{monthName11\}\}/img, MdsPersianDateTimePicker.getMonthName(10, calendarSystem));
    html = html.replace(/\{\{monthName12\}\}/img, MdsPersianDateTimePicker.getMonthName(11, calendarSystem));
    html = html.replace(/\{\{weekDayShortName1\}\}/img, MdsPersianDateTimePicker.getWeekDayShortName(0, calendarSystem));
    html = html.replace(/\{\{weekDayShortName2\}\}/img, MdsPersianDateTimePicker.getWeekDayShortName(1, calendarSystem));
    html = html.replace(/\{\{weekDayShortName3\}\}/img, MdsPersianDateTimePicker.getWeekDayShortName(2, calendarSystem));
    html = html.replace(/\{\{weekDayShortName4\}\}/img, MdsPersianDateTimePicker.getWeekDayShortName(3, calendarSystem));
    html = html.replace(/\{\{weekDayShortName5\}\}/img, MdsPersianDateTimePicker.getWeekDayShortName(4, calendarSystem));
    html = html.replace(/\{\{weekDayShortName6\}\}/img, MdsPersianDateTimePicker.getWeekDayShortName(5, calendarSystem));
    html = html.replace(/\{\{weekDayShortName7\}\}/img, MdsPersianDateTimePicker.getWeekDayShortName(6, calendarSystem));
    ...
    dateTimeToShowJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(selectedDateToShowTemp, calendarSystem);
    todayDateTimeJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(new Date(), calendarSystem);
    firstWeekDayNumber = MdsPersianDateTimePicker.getDateTimeJsonByCalendar2(dateTimeToShowJson.year, dateTimeToShowJson.month, 1, 0, 0, 0, calendarSystem).dayOfWeek;
    selectedDateNumber = !selectedDateTime ? 0 : MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(selectedDateTime, calendarSystem));
    numberOfDaysInCurrentMonth = MdsPersianDateTimePicker.getDaysInMonthByCalendar(dateTimeToShowJson.year, dateTimeToShowJson.month, calendarSystem);
    const previousMonthDateTimeJson = MdsPersianDateTimePicker.addMonthToDateTimeJson(dateTimeToShowJson, -1, calendarSystem);
    numberOfDaysInPreviousMonth = MdsPersianDateTimePicker.getDaysInMonthByCalendar(previousMonthDateTimeJson.year, previousMonthDateTimeJson.month, calendarSystem);
    previousMonthDateNumber = MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(MdsPersianDateTimePicker.getLastDayDateOfPreviousMonth(selectedDateToShowTemp, calendarSystem), calendarSystem));
    nextMonthDateNumber = MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(MdsPersianDateTimePicker.getFirstDayDateOfNextMonth(selectedDateToShowTemp, calendarSystem), calendarSystem));
    if (calendarSystem === 'gregorian') {
      ...
      rangeSelectorStartDateNumber = !setting.rangeSelector || !rangeSelectorStartDate ? 0 : MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(rangeSelectorStartDate, calendarSystem));
      rangeSelectorEndDateNumber = !setting.rangeSelector || !rangeSelectorEndDate ? 0 : MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(rangeSelectorEndDate, calendarSystem));
      for (i = 1; i <= 12; i++) {
        monthsDateNumberAndAttr['month' + i.toString() + 'DateNumber'] = MdsPersianDateTimePicker.convertToNumber2(dateTimeToShowJson.year, i, MdsPersianDateTimePicker.getDaysInMonthByCalendar(dateTimeToShowJson.year, i, calendarSystem));
      }
    }
    for (i = 0; i < setting.holidays.length; i++) {
      holidaysDateNumbers.push(MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(setting.holidays[i], calendarSystem)));
    }
    for (i = 0; i < setting.disabledDates.length; i++) {
      disabledDatesNumber.push(MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(setting.disabledDates[i], calendarSystem)));
    }
    for (i = 0; i < setting.specialDates.length; i++) {
      specialDatesNumber.push(MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(setting.specialDates[i], calendarSystem)));
    }

    let todayDateNumber = MdsPersianDateTimePicker.convertToNumber1(todayDateTimeJson);
    let selectedYear = MdsPersianDateTimePicker.isRtlCalendar(calendarSystem) ? MdsPersianDateTimePicker.toPersianNumber(dateTimeToShowJson.year) : dateTimeToShowJson.year.toString();
    ...
    let currentMonthInfo = MdsPersianDateTimePicker.getMonthName(dateTimeToShowJson.month - 1, calendarSystem) + ' ' + dateTimeToShowJson.year.toString();
    if (MdsPersianDateTimePicker.isRtlCalendar(calendarSystem))
      currentMonthInfo = MdsPersianDateTimePicker.toPersianNumber(currentMonthInfo);
    selectedMonthName = MdsPersianDateTimePicker.getMonthName(dateTimeToShowJson.month - 1, calendarSystem);
    ...
    // روز های ماه قبل
    if (!MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) && firstWeekDayNumber != 6 || MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) && firstWeekDayNumber != 0) {
      if (MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem))
        firstWeekDayNumber--;
      for (i = numberOfDaysInPreviousMonth - firstWeekDayNumber; i <= numberOfDaysInPreviousMonth; i++) {
        ...
        dayNumberInString = MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.zeroPad(i) : MdsPersianDateTimePicker.toPersianNumber(MdsPersianDateTimePicker.zeroPad(i));
        td = document.createElement('TD');
        ...
        // روز جمعه
        if (!MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) && tdNumber == 6)
          td.classList.add('text-danger');
        // روز یکشنبه
        else if (MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) && tdNumber == 0)
          td.classList.add('text-danger');
        tr.appendChild(td);
        cellNumber++;
        tdNumber++;
    ...
    for (i = 1; i <= numberOfDaysInCurrentMonth; i++) {
      ...
      if (currentDateNumber == todayDateNumber) {
        ...
        td.setAttribute('title', MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.todayText : MdsPersianDateTimePicker.todayTextPersian);
        // اگر نام روز هفته انتخاب شده در تکس باکس قبل از تاریخ امروز باشد
        // نباید دیگر نام روز هفته تغییر کند
        if (!dayOfWeek)
          dayOfWeek = MdsPersianDateTimePicker.getWeekDayName(tdNumber - 1 < 0 ? 0 : tdNumber - 1, calendarSystem);
      }
    ...
    // روزهای ماه بعد
    let nextMonthDateTimeJson = MdsPersianDateTimePicker.addMonthToDateTimeJson(dateTimeToShowJson, 1, calendarSystem);
  ...
  private getPopoverHeaderTitle(setting: MdsPersianDateTimePickerSetting): string {
    ...
    let title = '';
    selectedDateToShowJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(setting.selectedDateToShow, calendarSystem);
    if (setting.rangeSelector) {
      const startDate = MdsPersianDateTimePicker.addMonthToDateTime(setting.selectedDateToShow, -setting.rangeSelectorMonthsToShow[0], calendarSystem);
      const endDate = MdsPersianDateTimePicker.addMonthToDateTime(setting.selectedDateToShow, setting.rangeSelectorMonthsToShow[1], calendarSystem);
      const statDateJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(startDate, calendarSystem);
      const endDateJson = MdsPersianDateTimePicker.getDateTimeJsonByCalendar(endDate, calendarSystem);
      const startMonthName = MdsPersianDateTimePicker.getMonthName(statDateJson.month - 1, calendarSystem);
      const endMonthName = MdsPersianDateTimePicker.getMonthName(endDateJson.month - 1, calendarSystem);
      title = `${startMonthName} ${statDateJson.year} - ${endMonthName} ${endDateJson.year}`;
    }
    else
      title = `${MdsPersianDateTimePicker.getMonthName(selectedDateToShowJson.month - 1, calendarSystem)} ${selectedDateToShowJson.year}`;
    if (MdsPersianDateTimePicker.isRtlCalendar(calendarSystem))
  ...
  private getDateTimePickerBodyHtml(setting: MdsPersianDateTimePickerSetting): string {
    ...
    html = html.replace(/\{\{goTodayText\}\}/img, MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? MdsPersianDateTimePicker.goTodayText : MdsPersianDateTimePicker.goTodayTextPersian);
    ...
    todayDateString = `${MdsPersianDateTimePicker.isGregorianCalendar(calendarSystem) ? 'Today,' : 'نن،'} ${todayDateTimeJson.day} ${MdsPersianDateTimePicker.getMonthName(todayDateTimeJson.month - 1, calendarSystem)} ${todayDateTimeJson.year}`;
    if (MdsPersianDateTimePicker.isRtlCalendar(calendarSystem)) {
    ...
    if (disableAfterDateTimeJson != undefined && disableAfterDateTimeJson.year <= selectedDateTimeToShowJson.year && disableAfterDateTimeJson.month < selectedDateTimeToShowJson.month)
      selectedDateToShow = MdsPersianDateTimePicker.getDateTimeByCalendar(disableAfterDateTimeJson.year, disableAfterDateTimeJson.month, 1, calendarSystem);
    ...
    if (disableBeforeDateTimeJson != undefined && disableBeforeDateTimeJson.year >= selectedDateTimeToShowJson.year && disableBeforeDateTimeJson.month > selectedDateTimeToShowJson.month)
      selectedDateToShow = MdsPersianDateTimePicker.getDateTimeByCalendar(disableBeforeDateTimeJson.year, disableBeforeDateTimeJson.month, 1, calendarSystem);
    ...
    for (let i1 = numberOfPrevMonths; i1 < 0; i1++) {
      setting.selectedDateToShow = MdsPersianDateTimePicker.addMonthToDateTime(MdsPersianDateTimePicker.getClonedDate(selectedDateToShow), i1, calendarSystem);
    ...
    for (let i2 = 1; i2 <= numberOfNextMonths; i2++) {
      setting.selectedDateToShow = MdsPersianDateTimePicker.addMonthToDateTime(MdsPersianDateTimePicker.getClonedDate(selectedDateToShow), i2, calendarSystem);
  ...
  private changeMonth = (element: Element): void => {
    ...
    const calendarSystem = MdsPersianDateTimePicker.getCalendarSystem(setting);
    let selectedDateToShow = MdsPersianDateTimePicker.getClonedDate(setting.selectedDateToShow);
    selectedDateToShow = MdsPersianDateTimePicker.getDateTime4(dateNumber, selectedDateToShow, calendarSystem);
    setting.selectedDateToShow = MdsPersianDateTimePicker.getClonedDate(selectedDateToShow);
  ...
  private hoverOnDays = (e: Event): void => {
    ...
    if (calendarSystem === 'gregorian') {
      ...
      rangeSelectorStartDateNumber = !rangeSelectorStartDate ? 0 : MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(rangeSelectorStartDate, calendarSystem));
      rangeSelectorEndDateNumber = !rangeSelectorEndDate ? 0 : MdsPersianDateTimePicker.convertToNumber1(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(rangeSelectorEndDate, calendarSystem));
    }
  ...
  */
  ...
  /**
  * بروز کردن تاریخ انتخاب شده با استفاده از تاریخ اسلامی
  */
  setDateIslamic(yearIslamic: number, monthIslamic: number, dayIslamic: number): void {
    const gregorianDateJson = MdsPersianDateTimePicker.islamicToGregorian(yearIslamic, monthIslamic, dayIslamic);
    const date = new Date(gregorianDateJson.gy, gregorianDateJson.gm - 1, gregorianDateJson.gd);
    this.updateOptions({
 ...
 * تبدیل آبجکت تاریخ به رشته
  ...
  */
  static convertDateToString = (date: Date, isGregorian: boolean, format: string): string => {
    const calendarSystem: CalendarSystem = isGregorian ? 'gregorian' : 'jalali';
    return MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(date, calendarSystem), format, calendarSystem, !isGregorian);
  };
  /**
 * تبدیل آبجکت تاریخ به رشته با انتخاب سیستم تقویم
 */
  static convertDateToStringByCalendar = (date: Date, calendarSystem: CalendarSystem, format: string, persianNumber = false): string => {
    return MdsPersianDateTimePicker.getDateTimeString(MdsPersianDateTimePicker.getDateTimeJsonByCalendar(date, calendarSystem), format, calendarSystem, persianNumber);
  };
  /**
 ...
 */
  ...
  /**
 * تبدیل آبجکت تاریخ به اسلامی
 */
  static convertDateToIslamic = (date: Date): MdsPersianDateTimePickerConvertedDateModel => {
    const dateTimeJson1 = MdsPersianDateTimePicker.getDateTimeJson1(date);
    const islamicJsonModel = MdsPersianDateTimePicker.toIslamic(dateTimeJson1.year, dateTimeJson1.month, dateTimeJson1.day);
    return {
      year: islamicJsonModel.iy,
      month: islamicJsonModel.im,
      day: islamicJsonModel.id,
    }
  };
  /**
